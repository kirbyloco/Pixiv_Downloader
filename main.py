...
def getjson():
    urlID = input('請輸入作者ID：')
    print('正在收集圖片URL')
    Url = f'https://www.pixiv.net/ajax/user/{urlID}/profile/all'
    IDjson = login_session.get(Url).json()
...
def tagcheck(urlID):
    Url = f'https://www.pixiv.net/ajax/user/{urlID}/illustmanga/tag?tag=R-18&offset=0&limit=9999'
    v = login_session.get(Url).json()
    tag = v['body']['works']
    for total in tag:
